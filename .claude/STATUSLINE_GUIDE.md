...
**第一行：** 模型icon+名 │ 水位icon+Context% (token數) │ 📂目錄 🌿分支 │ ⏱時間 │ 💰花費
...
pink='\033[38;2;255;82;82m'       # Signal Red #FF5252
dim='\033[2m'                     # 淡��
reset='\033[0m'                   # 重置
```
...
| 顏色 | 名稱 | Hex | R | G | B | 用途 |
|------|------|-----|---|---|---|------|
| 紅 | Tesla Red | #E82127 | 232 | 33 | 39 | Opus 模型、危險 |
| 藍 | Electric Blue | #38ACFF | 56 | 172 | 255 | Sonnet 模型 |
| 綠 | Neon Green | #00E676 | 0 | 230 | 118 | Haiku、低使用率、Git 分支 |
...
2. 水位+Context%  ${level_icon} ${pct_color}${pct_used}%${reset} (token數)
3. 📂 目錄名      ${cyan}📂 ${dirname}${reset}
4. 🌿 Git 分支    🌿 ${git_branch} 💫(dirty)  ← 有 git 才顯示
