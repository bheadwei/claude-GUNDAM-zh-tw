...
**必讀規範：** `.claude/skills/plan-format/SKILL.md`；規劃含 API 的功能時加讀 `.claude/skills/api-design/SKILL.md`（設計或改 API 介面前）
...
4. **若規劃過程做了非顯而易見的技術選型**（選了某個函式庫/模式，且有評估過替代方案）
   → 提議使用者跑 `/adr` 記錄決策與被否決的方案。判準：三個月後有人問
   「為什麼不用 X？」答不出來就該記。**不要**為「只有一條路可走」的事提議。
