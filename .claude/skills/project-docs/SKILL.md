---
name: project-docs
description: 依據 VibeCoding 工作流範本撰寫專案文件。涵蓋 PRD、架構設計、ADR、API 規範、WBS 等 17 種文件範本，支援完整流程與 MVP 兩種模式。適用於需要產出專案文件、規劃文檔或技術規格時。
---

# 專案文件撰寫

依據 `VibeCoding_Workflow_Templates/` 中的 17 種範本，產出結構化的專案文件。
...
| 階段 | 編號 | 範本 | 用途 | 適用模式 |
|:---|:---|:---|:---|:---|
| **規劃** | 02 | `project_brief_and_prd.md` | 專案簡報與 PRD | 完整+MVP |
...
| **詳設** | 07 | `module_specification_and_tests.md` | 模組規格與測試 | 完整 |
| | 08 | `project_structure_guide.md` | 專案結構指南 | 完整+MVP |
| | 09 | `file_dependencies_template.md` | 模組依賴分析 | 完整 |
| | 10 | `class_relationships_template.md` | 類別關係（UML） | 完整 |
| **開發** | 11 | `code_review_and_refactoring_guide.md` | 程式碼審查指南 | 完整 |
...
```
02 PRD → 03 BDD → 04 ADR + 05 架構 → 06 API + 07 模組 → 08 結構 + 09 依賴 + 10 類別 → 11 審查 + 12/17 前端 → 13 安全 → 14 部署 → 15 文檔 + 16 WBS
```
...
依範本欄位，逐項向使用者收集：
- 用 `AskUserQuestion` 一次問 1-2 個關鍵欄��
...
撰寫時自動檢查：
- PRD (02) 的 User Story ID → 是否在架構 (05) 中被引用
...
3. **量化優於描述** — KPI 用數字、NFR 用目標值、工時用小時
...
| Tech Lead | 04 ADR、05 架構、06 API |
| 架構師 | 05 架構、09 依賴、10 類別 |
