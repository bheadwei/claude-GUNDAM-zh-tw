...
不確定該貼哪些檔案或哪些坑時，**先自己 grep 一輪確認這個專案用的字**——
你搜「rate limit」搜不到，往往是因為這個 codebase 叫它 `throttle`。
第一輪搜尋真正的產物常常是詞彙而不是檔案，拿新詞再搜一次。
**最多三輪就停**，剩下的缺口在 prompt 裡明講「這部分你自己找」，
不要為了湊完整把半個 repo 讀進你的 context。
...
| 疊一串「不要寫過度防禦／不要測型別」 | 它會對所有測試變保守，連該寫的邊界測試也不寫。改用審查後的清理階段（見 SKILL.md） |
