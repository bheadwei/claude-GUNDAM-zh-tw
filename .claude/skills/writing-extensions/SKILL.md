...
新增 rule 前的三道判準（第二、三道比第一道鋒利，也比較常被漏）：

| 問 | 答「否」就別寫 |
|---|---|
| **任何任務都適用嗎？** | 只在特定情境成立 → 做成 skill |
| **模型本來就不會做嗎？** | 模型預設就會做 → 寫進來是**純粹的注意力稀釋**。它不會讓行為變好，只會讓旁邊那些真正違反直覺的規則更容易被跳過 |
| **這件事別處已經有唯一來源了嗎？** | 有 → 寫指標，不要抄一份（見上一節） |

第二道判準取自 Godzilla-z 模板的 `rules/git-workflow.md`——它開頭寫著「本檔只留
**每次 git 操作都成立、而且與模型預設行為不同**的約束」，比只問「適用範圍多廣」鋒利。

第三道判準在 rule 上特別容易破功，而且有前例：`rules/git-workflow.md` 第一版
**整檔被刪掉**（commit `7596b1d`），因為兩節內容分別重述了 `commands/pr.md` 與
`coding-style.md`。2026-09-09 重寫時只留三條真正的鐵律，並在檔頭寫下這段歷史，
讓下一個想把 PR 內容補回去的人先看到警告——**新版本身就是這三道判準的範例**。

---

## 第三個問題：怎麼知道它真的生效

**這是這個模板最薄弱的一環。** 232 個 hook 測試證明的是閘門邏輯正確，
**完全測不到「模型會不會照做」**。

2026-09-09 第一輪實跑證實了這件事的重要性：文件維護路由（`documentation-specialist`）
兩份 prompt 都沒走，而它正是 `using-taskmaster` 的 Red Flags 表標為
「最常被漏掉的一棒」的那條。反向題則 2/2 通過——注入沒有硬到壓過使用者的明確指示。
判讀與 harness 的三個限制見 `.claude/tests/skill-compliance/README.md`。
