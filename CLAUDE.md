...
- **改了注入內容**（`using-taskmaster`、關鍵字路由、rules）→ 跑一輪 `.claude/tests/skill-compliance/run-compliance.sh`。188 個 hook 測試證明不了「模型會不會照做」
...
- **壓力測試從未實跑真實對話**：`run-compliance.sh` 的機制驗過（含新加的 `-n K`／pass@k），但沒跑過一輪真的對話。188 個 hook 測試證明閘門邏輯，證明不了「模型會不會照做」——**這仍是最大的驗證盲區**
